...
(define nrepl-client (make-nrepl-client-socket #:port 42821))
(scm->bencode
 `(("op" . "ls-sessions"))
 nrepl-client)
(scm->bencode
...
(define sample-expression '(lambda (x) (+ 3 x 4)))

(use-modules (language cps))
(unparse-cps (compile sample-expression #:to 'cps-cps))

;; (compile '(eq 1 2) #:from 'elisp #:to 'tree-il)
(unparse-tree-il (compile  #:to 'tree-il))

(lookup-language)
(gensym "prefix-")


(tree-il->scheme (macroexpand '(some-macro)))
