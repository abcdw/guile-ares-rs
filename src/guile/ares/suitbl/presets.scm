...
(define-module (ares suitbl presets)
  #:use-module ((ares suitbl definitions) #:select (current-test-runner))
...
(define* (runner->state #:optional (runner (current-test-runner)))
...
(define* (only-slow! #:optional (runner (current-test-runner)))
...
(define* (only-fast! #:optional (runner (current-test-runner)))
  "Configure RUNNER to schedule only fast tests."
  (let ((state (runner->state runner)))
    (set-runner-config-value! state 'schedule-tests scheduler:fast)
    (get-runner-config state)))

(define* (matching! pattern #:optional (runner (current-test-runner)))
...
(define* (rerun-failed-or-all! #:optional (runner (current-test-runner)))
...
(define* (raise-on-error! #:optional (runner (current-test-runner)))
...
(define* (reset! #:optional (runner (current-test-runner)))
